...
#include <std_msgs/Float32.h>   // ★ 추가: YOLO 토픽용
...
// 기존 LiDAR 플래그용 (남겨두되, 지금은 사용 안 해도 됨)
...
// /yolo/bbox_x (Float32)
void bboxXCB(const std_msgs::Float32::ConstPtr& msg) {
  bbox_x_ = msg->data;
  has_bbox_info_ = true;
}

// /yolo/bbox_height_ratio (Float32, % 단위)
void bboxHeightCB(const std_msgs::Float32::ConstPtr& msg) {
  bbox_height_ratio_ = msg->data;
  has_bbox_info_ = true;
}
...
int main(int argc, char **argv) {
  ...
  pnh.param<double>("avoid_kmh",       avoid_kmh_,     12.0);
  ...
  // ★ YOLO 기반 회피를 위한 구독
  ros::Subscriber bbox_x_sub =
      nh.subscribe("/yolo/bbox_x", 1, bboxXCB);
  ros::Subscriber bbox_h_sub =
      nh.subscribe("/yolo/bbox_height_ratio", 1, bboxHeightCB);
  ...
  while (ros::ok()) {
    ...
    if (mode_ == Mode::AVOID_LEFT || mode_ == Mode::AVOID_RIGHT) {
      if (avoid_first_step_hard_right_) {
        ...
        else                            cmd_.steering = 0.2;  // AVOID_LEFT
      }
